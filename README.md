...
- 📦 کالکشن کامل Postman (۱۳۶ درخواست) در فایل `Building Management Pro - Master.postman_collection`
...
- **کالکشن Postman:** فایل `Building Management Pro - Master.postman_collection` — شامل ۱۳۶ درخواست در ۷ پوشه، متغیر `base_url`، ذخیره خودکار توکن بعد از Login و هدرهای خودکار `Accept` و `Content-Type: application/json`
...
```bash
composer test            # ۶۵۵ تست یکپارچه + E2E (بدون نیاز به دیتابیس یا افزونه خاص)
...
| لاگ‌گیر مرکزی و ۶۵۵ تست (یکپارچه + رندر E2E صفحات) | ✅ تکمیل |
