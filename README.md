...
- 📦 کالکشن کامل Postman (۱۳۱ درخواست) در فایل `Building Management Pro - Master.postman_collection`
...
- **کالکشن Postman:** فایل `Building Management Pro - Master.postman_collection` — شامل ۱۳۱ درخواست در ۷ پوشه، متغیر `base_url`، ذخیره خودکار توکن بعد از Login و هدرهای خودکار `Accept` و `Content-Type: application/json`
...
```bash
composer test            # ۴۶۸ تست یکپارچه (بدون نیاز به دیتابیس یا افزونه خاص)
...
| لاگ‌گیر مرکزی و ۴۶۸ تست یکپارچه | ✅ تکمیل |
