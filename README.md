...
- 📦 کالکشن کامل Postman (۱۴۱ درخواست) در فایل `Building Management Pro - Master.postman_collection`
...
- **هزینهٔ دوره‌ای با تناوب دلخواه**: هفتگی، دوهفتگی، ماهانه، دوماهانه، سه‌ماهانه یا سالانه با تاریخ شروع/پایان؛ کران روزانه در هر نوبت یک هزینهٔ جدا می‌سازد و برای مخاطبان صادر می‌کند (کاملاً توان‌تکرار)
- **حسابداری حرفه‌ای واحد/شخص**: ماندهٔ بدهکار/طلبکار هر واحد به تفکیک گردش حساب (لجر) با ماندهٔ لحظه‌ای؛ پرداخت بیشتر از بدهی = طلبکاری و کسر خودکار از بدهی‌های بعدی
- **پرداخت و بدهی مستقیم مدیر**: ثبت پرداخت برای واحد بدون درخواست ساکن (تأیید فوری) و ثبت بدهی یک‌باره برای یک واحد
- **تقسیم داینامیک هزینه**: سهم ثابت (`fixed_share`)، بر اساس متراژ (`area`)، بر اساس تعداد ساکنان (`people_count`)
- جریان کامل پرداخت: `ثبت با فیش اجباری → در انتظار تأیید → تأیید مدیر`
- **فیش واریزی اجباری** هنگام ثبت پرداخت؛ ذخیرهٔ امن در پوشهٔ اختصاصی ساختمان و واحد (`storage/buildings/{bid}/receipts/unit-{slug}/`)، نمایان فقط برای پرداخت‌کننده و مدیر همان ساختمان و بدون دسترسی مستقیم از بیرون
- آپلود مجدد فیش پس از رد پرداخت (حداکثر ۵ مگابایت، تصویر)
...
├── building_add.php           # ثبت ساختمان (building_view.php → ریدایرکت به داشبورد)
...
|---|---|---|
| `MELIPAYAMAK_USERNAME` | — | نام‌کاربری پنل پیامک — بدون این سه متغیر پیامک «غیرفعال» است و ارسال‌ها بی‌صدا رد می‌شوند |
| `MELIPAYAMAK_PASSWORD` | — | رمز پنل پیامک |
| `MELIPAYAMAK_BODY_ID` | — | شناسه بدنه (پترن) |
...
| `MELIPAYAMAK_DEBTOR_BODY_ID` | — | پترن جداگانه پیامک یادآوری **بدهکاران** (اختیاری؛ آرگومان‌ها به‌ترتیب: نام، مبلغ، نام ساختمان، مهلت). اگر تنظیم نشود، متن کامل با پترن پیش‌فرض ارسال می‌شود |
| `SMS_DEBTOR_MIN_AMOUNT` | `10000` | حداقل بدهی (تومان) برای ورود به فهرست یادآوری پیامکی |
| `SMS_DEBTOR_TEMPLATE` | قالب پیش‌فرض | متن پیامک بدهی با جای‌دارهای `{نام} {مبلغ} {ساختمان} {مهلت}` |
...
- برای اطمینان، کران روزانهٔ `scripts/cron_daily.php` نیز شارژ ماه جاری همهٔ ساختمان‌های فعال را تضمین می‌کند (بخش «شارژ و هزینه‌های دوره‌ای» را ببینید).
...
### شارژ و هزینه‌های دوره‌ای (کران روزانه)

- اسکریپت `scripts/cron_daily.php` هر روز:
  - برای ساختمان‌های دارای «شارژ ماهیانه» فعال، شارژ ماه جاری را می‌سازد و صادر می‌کند (ماه‌های عقب‌افتاده هم پوشش داده می‌شوند).
  - هزینه‌های دوره‌ای با تناوب دلخواه را که نوبتشان رسیده نمونه‌سازی و صادر می‌کند.
  - برای واحدهای **بدهکار** پیامک یادآوری می‌فرستد (هر واحد حداکثر یک‌بار در هر ماه شمسی — کلید یکتای واحد+دوره در جدول `debtor_sms_log`). اگر اعتبار پیامک تنظیم نباشد، این مرحله بی‌صدا رد می‌شود.
- هر نوبت با نشانگر یکتا (`auto:recurring:{id}:{date}`) ثبت می‌شود؛ اجرای مکرر کران هزینهٔ تکراری **نمی‌سازد**.
- اگر صدور یک نوبت شکست بخورد (مثلاً واحدی ساکن ندارد)، نوبت حذف می‌شود و سررسید ثابت می‌ماند تا در اجرای بعدی دوباره تلاش شود.
- مدیر می‌تواند از صفحهٔ «حسابداری» همان لحظه دکمهٔ «اجرای موتور دوره‌ای» را بزند (بدون انتظار برای کران).

```bash
php scripts/cron_daily.php
# کرون روزانه ساعت ۷ صبح (پیشنهادی):
# 0 7 * * * /usr/bin/php /path/to/scripts/cron_daily.php >> /path/to/storage/cron.log 2>&1
```

### حسابداری ساختمان

- صفحهٔ `accounting.php` (دسترسی مدیر): جدول ماندهٔ واحدها با حالت بدهکار/طلبکار/تسویه، گردش حساب کامل هر واحد (بدهی/پرداخت با ماندهٔ لحظه‌ای) و مانده به تفکیک شخص.
- ماندهٔ مثبت یعنی ساختمان به واحد بدهکار است؛ بدهی جدید ابتدا از اعتبار واحد کم می‌شود.
- اندپوینت‌ها: `GET /api/buildings/{id}/ledger`، `POST /api/buildings/{id}/direct-payments`، `POST /api/buildings/{id}/unit-charges`، `POST /api/buildings/{id}/recurring-generate`
...
```bash
php scripts/migrator.php   # همهٔ مهاجرت‌ها (تا 036: صندوق پیام، ایندکس‌های کارایی و لاگ یادآوری بدهکاران)
```
...
| 010 | `cost_payments` | پرداخت‌ها (وضعیت: در انتظار تأیید / تأییدشده / ردشده؛ ثبت با فیش واریزی اجباری) |
...
| 033 | `costs.recurring_start_date`, `costs.recurring_end_date`, `costs.recurring_next_date`, `costs.parent_cost_id` | هزینه‌های دوره‌ای با تناوب دلخواه و اتصال نمونه‌های صادرشده به قالب |
...
| POST | `/payments/submit` | ارسال درخواست پرداخت (multipart) — `cost_id`, `payment_id`, `amount_paid`, `notes` + فیلد فایل **الزامی** `receipt` |
| GET | `/payments/{payment_id}` | جزئیات یک پرداخت (پرداخت‌کننده یا مدیر ساختمان) |
...
| مشاهده ساختمان | `dashboard.php` | هاب یگانهٔ ساختمان: جزئیات + نمای گرافیکی بلوک/طبقه/واحد با پاپ‌آپ اطلاعات و اقدامات واحد (صفحهٔ `building_view.php` با داشبورد یکپارچه شده و فقط ریدایرکت می‌کند) |
...
- ✅ **Rate Limiting** چندلایه بر اساس IP + الگوی مسیر (Redis، fail-open) — `RateLimitMiddleware`:
  سقف سراسری هر IP، سقف هر مسیر، و محدودیت‌های سخت‌گیرانه برای ارسال/تأیید کد پیامک،
  ورود/ثبت‌نام/تغییر رمز و ثبت پرداخت؛ ارسال پیامک بر اساس شمارهٔ مقصد هم محدود می‌شود
...
```bash
composer test            # ۹۵۰ تست یکپارچه + E2E (بدون نیاز به دیتابیس یا افزونه خاص)
...
| لاگ‌گیر مرکزی و ۹۵۰ تست (یکپارچه + رندر E2E صفحات) | ✅ تکمیل |
...
| ساخت گروهی کاربران: جدول بالک با چسباندن از اکسل، اتصال به واحد، رمز پیش‌فرض، گزارش ردیف‌به‌ردیف | ✅ تکمیل |
| شارژ ترکیبی (مبلغ ثابت + سهم هر نفر) در کنار حالت‌های ثابت/نفری/دلخواه | ✅ تکمیل |
...
| حالت شب/روز (🌙) با ذخیرهٔ انتخاب کاربر و اعمال بدون فلش در همهٔ صفحات | ✅ تکمیل |
| صندوق پیام خصوصی درون‌اپی (چت مدیر ↔ ساکن، گفتگوها، نشان نخوانده، اعلان دریافت) | ✅ تکمیل |
| نمایش شمارهٔ پارکینگ و انباری در کارت اعضا و رسید پرداخت‌ها | ✅ تکمیل |
...
| داشبورد تحلیلی مدیر: نمودار روند ۶ ماههٔ درآمد/هزینه، درصد وصولی و بدهکارترین واحدها (بدون کتابخانهٔ خارجی) | ✅ تکمیل |
| استانداردسازی API: هم‌نامی `/api/v1/…`، صفحه‌بندی یکپارچهٔ اختیاری و لاگ ساختاریافتهٔ درخواست‌ها | ✅ تکمیل |
| صف کار پس‌زمینه (جدول `jobs` + کران): ارسال پیامک و اعلان خارج از مسیر درخواست، سازگار با هاست اشتراکی (مهاجرت ۰۳۷) | ✅ تکمیل |
| تور آنبوردینگ مدیر (بار اول) + حالت‌های خالی مصور + پولیش سراسری (توست، انتقال صفحات، میکروانیمیشن) | ✅ تکمیل |
| اعلان فوری وب (Web Push): نوتیفیکیشن لحظه‌ای روی گوشی با کلید VAPID و رمزنگاری RFC 8291، بدون کتابخانهٔ خارجی | ✅ تکمیل |
| اسکریپت دادهٔ نمونه `scripts/seed.php`: ساخت ساختمان کامل با اعضا، واحدها، شارژ و پرداخت برای دمو و تست | ✅ تکمیل |
